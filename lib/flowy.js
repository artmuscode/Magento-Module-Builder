var flowy = function(canvas, grab, release, snapping, rearrange, spacing_x, spacing_y, direction) {
    if (!grab) {
        grab = function() {};
    }
    if (!release) {
    ...
    var isHorizontal = direction === "right" || direction === "horizontal";
    ...
    flowy.load = function() {
        ...
        function checkAttach(id) {
            ...
            var parent = blocks.filter(a => a.id == id)[0];
            if (isHorizontal) {
                if (xpos >= parent.x - (parent.width / 2) && xpos <= parent.x + parent.width + paddingx && ypos >= parent.y - (parent.height / 2) - paddingy && ypos <= parent.y + (parent.height / 2) + paddingy) {
                    return true;
                }
                return false;
            }
            if (xpos >= parent.x - (parent.width / 2) - paddingx && xpos <= parent.x + (parent.width / 2) + paddingx && ypos >= parent.y - (parent.height / 2) && ypos <= parent.y + parent.height) {
                return true;   
            } else {
                return false;
            }
        ...
        function drawArrow(arrow, x, y, id) {
            if (isHorizontal) {
                var parent = blocks.filter(a => a.id == id)[0];
                canvas_div.innerHTML += '<div class="arrowblock"><input type="hidden" class="arrowid" value="' + drag.querySelector(".blockid").value + '"><svg preserveaspectratio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 20L' + (x / 2) + ' 20L' + (x / 2) + ' ' + (20 + y) + 'L' + x + ' ' + (20 + y) + '" stroke="#C5CCD0" stroke-width="2px"/><path d="M' + x + ' ' + (20 + y) + 'L' + (x - 8) + ' ' + (20 + y - 5) + 'L' + (x - 8) + ' ' + (20 + y + 5) + 'Z" fill="#C5CCD0"/></svg></div>';
                document.querySelector('.arrowid[value="' + parseInt(drag.querySelector(".blockid").value) + '"]').parentNode.style.left = parent.x + (parent.width / 2) - (absx + window.scrollX) + canvas_div.scrollLeft + canvas_div.getBoundingClientRect().left + "px";
                document.querySelector('.arrowid[value="' + parseInt(drag.querySelector(".blockid").value) + '"]').parentNode.style.top = parent.y - 20 - (absy + window.scrollY) + canvas_div.scrollTop + canvas_div.getBoundingClientRect().top + "px";
            } else {
                if (x < 0) {
        ...
        function updateArrow(arrow, x, y, children) { 
            if (isHorizontal) {
                var parent = blocks.filter(id => id.id == children.parent)[0];
                document.querySelector('.arrowid[value="' + children.id + '"]').parentNode.style.left = parent.x + (parent.width / 2) - (absx + window.scrollX) + canvas_div.getBoundingClientRect().left + "px";
                document.querySelector('.arrowid[value="' + children.id + '"]').parentNode.style.top = parent.y - 20 - (absy + window.scrollY) + canvas_div.getBoundingClientRect().top + "px";
                document.querySelector('.arrowid[value="' + children.id + '"]').parentNode.innerHTML = '<input type="hidden" class="arrowid" value="' + children.id + '"><svg preserveaspectratio="none" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 20L' + (x / 2) + ' 20L' + (x / 2) + ' ' + (20 + y) + 'L' + x + ' ' + (20 + y) + '" stroke="#C5CCD0" stroke-width="2px"/><path d="M' + x + ' ' + (20 + y) + 'L' + (x - 8) + ' ' + (20 + y - 5) + 'L' + (x - 8) + ' ' + (20 + y + 5) + 'Z" fill="#C5CCD0"/></svg>';
            } else {
                if (x < 0) {
        ...
        function snap(drag, i, blocko) {
            ...
            for (var w = 0; w < blocks.filter(id => id.parent == blocko[i]).length; w++) {
                ...
                if (isHorizontal) {
                    if (children.childwidth > children.height) {
                        totalwidth += children.childwidth + paddingy;
                    } else {
                        totalwidth += children.height + paddingy;
                    }
                } else {
            ...
            if (isHorizontal) {
                totalwidth += parseInt(window.getComputedStyle(drag).height);
            } else {
            ...
            for (var w = 0; w < blocks.filter(id => id.parent == blocko[i]).length; w++) {
                var children = blocks.filter(id => id.parent == blocko[i])[w];
                if (isHorizontal) {
                    var parent = blocks.filter(id => id.id == blocko[i])[0];
                    document.querySelector(".blockid[value='" + children.id + "']").parentNode.style.left = parent.x + (parent.width / 2) + paddingx - (window.scrollX + absx) + canvas_div.getBoundingClientRect().left + "px";
                    if (children.childwidth > children.height) {
                        document.querySelector(".blockid[value='" + children.id + "']").parentNode.style.top = parent.y - (totalwidth / 2) + totalremove + (children.childwidth / 2) - (children.height / 2) - (window.scrollY + absy) + canvas_div.getBoundingClientRect().top + "px";
                        children.y = parent.y - (totalwidth / 2) + totalremove + (children.childwidth / 2);
                        totalremove += children.childwidth + paddingy;
                    } else {
                        document.querySelector(".blockid[value='" + children.id + "']").parentNode.style.top = parent.y - (totalwidth / 2) + totalremove - (window.scrollY + absy) + canvas_div.getBoundingClientRect().top + "px";
                        children.y = parent.y - (totalwidth / 2) + totalremove + (children.height / 2);
                        totalremove += children.height + paddingy;
                    }
                    children.x = parent.x + (parent.width / 2) + paddingx + (children.width / 2);
                } else {
                    if (children.childwidth > children.width) {
            ...
            if (isHorizontal) {
                drag.style.left = blocks.filter(id => id.id == blocko[i])[0].x + (blocks.filter(id => id.id == blocko[i])[0].width / 2) + paddingx - (window.scrollX + absx) + canvas_div.scrollLeft + canvas_div.getBoundingClientRect().left + "px";
                drag.style.top = blocks.filter(id => id.id == blocko[i])[0].y - (totalwidth / 2) + totalremove - (window.scrollY + absy) + canvas_div.getBoundingClientRect().top + "px";
            } else {
            ...
            var arrowx = isHorizontal ? paddingx : arrowblock.x - blocks.filter(a => a.id == blocko[i])[0].x + 20;
            var arrowy = isHorizontal ? (arrowblock.y - blocks.filter(a => a.id == blocko[i])[0].y) : paddingy;
                            ...
                            if (isHorizontal) {
                                if (children.childwidth > children.height) {
                                    if (w == blocks.filter(id => id.parent == idval).length - 1) {
                                        ...
                                        zwidth += children.childwidth + paddingy;
                                    }
                                } else {
                                    if (w == blocks.filter(id => id.parent == idval).length - 1) {
                                        zwidth += children.height;
                                    } else {
                                        zwidth += children.height + paddingy;
                                    }
                                }
                            } else {
                                if (children.childwidth > children.width) {
            ...
            if (active || rearrange) {
                ...
                for (var i = 0; i < blocks.length; i++) {
                    if (checkAttach(blocko[i])) {
                        ...
                        if (isHorizontal) {
                            document.querySelector(".indicator").style.left = document.querySelector(".blockid[value='" + blocko[i] + "']").parentNode.offsetWidth + "px";
                            document.querySelector(".indicator").style.top = (document.querySelector(".blockid[value='" + blocko[i] + "']").parentNode.offsetHeight / 2) - 5 + "px";
                        } else {
        ...
        function checkOffset() {
            if (isHorizontal) {
                var offsettop = blocks.map(a => a.y);
                var heights = blocks.map(a => a.height);
                var mathtop = offsettop.map(function(item, index) {
                    return item - (heights[index] / 2);
                });
                offsettop = Math.min.apply(Math, mathtop);
                if (offsettop < (canvas_div.getBoundingClientRect().top + window.scrollY - absy)) {
                    var blocko = blocks.map(a => a.id);
                    for (var w = 0; w < blocks.length; w++) {
                        document.querySelector(".blockid[value='" + blocks.filter(a => a.id == blocko[w])[0].id + "']").parentNode.style.top = blocks.filter(a => a.id == blocko[w])[0].y - (blocks.filter(a => a.id == blocko[w])[0].height / 2) - offsettop + canvas_div.getBoundingClientRect().top - absy + 20 + "px";
                        if (blocks.filter(a => a.id == blocko[w])[0].parent != -1) {
                            var arrowblock = blocks.filter(a => a.id == blocko[w])[0];
                            document.querySelector('.arrowid[value="' + blocko[w] + '"]').parentNode.style.top = blocks.filter(a => a.id == blocks.filter(a => a.id == blocko[w])[0].parent)[0].y - 20 - offsettop + canvas_div.getBoundingClientRect().top - absy + 20 + "px";
                        }
                    }
                    for (var w = 0; w < blocks.length; w++) {
                        blocks[w].y = (document.querySelector(".blockid[value='" + blocks[w].id + "']").parentNode.getBoundingClientRect().top + window.scrollY) + (canvas_div.scrollTop) + (parseInt(window.getComputedStyle(document.querySelector(".blockid[value='" + blocks[w].id + "']").parentNode).height) / 2) - 20 - canvas_div.getBoundingClientRect().top;
                    }
                }
                return;
            }
                    ...
                    for (var w = 0; w < blocks.filter(id => id.parent == result[z]).length; w++) {
                        ...
                        if (children.childwidth > children.height) {
                            if (w == blocks.filter(id => id.parent == result[z]).length - 1) {
                                totalheight += children.childwidth;
                            } else {
                                totalheight += children.childwidth + paddingy;
                            }
                        } else {
                            if (w == blocks.filter(id => id.parent == result[z]).length - 1) {
                                totalheight += children.height;
                            } else {
                                totalheight += children.height + paddingy;
                            }
                        }
                    }
                    if (result[z] != -1) {
                        blocks.filter(a => a.id == result[z])[0].childwidth = totalheight;
                    }
                    for (var w = 0; w < blocks.filter(id => id.parent == result[z]).length; w++) {
                        var children = blocks.filter(id => id.parent == result[z])[w];
                        const r_block = document.querySelector(".blockid[value='" + children.id + "']").parentNode;
                        const r_parent = blocks.filter(id => id.id == result[z])[0];
                        r_block.style.left = r_parent.x + (r_parent.width / 2) + paddingx - (absx + window.scrollX) + canvas_div.getBoundingClientRect().left + "px";
                        if (children.childwidth > children.height) {
                            r_block.style.top = r_parent.y - (totalheight / 2) + totalremove + (children.childwidth / 2) - (children.height / 2) - (absy + window.scrollY) + canvas_div.getBoundingClientRect().top + "px";
                            children.y = r_parent.y - (totalheight / 2) + totalremove + (children.childwidth / 2);
                            totalremove += children.childwidth + paddingy;
                        } else {
                            r_block.style.top = r_parent.y - (totalheight / 2) + totalremove - (absy + window.scrollY) + canvas_div.getBoundingClientRect().top + "px";
                            children.y = r_parent.y - (totalheight / 2) + totalremove + (children.height / 2);
                            totalremove += children.height + paddingy;
                        }
                        children.x = r_parent.x + (r_parent.width / 2) + paddingx + (children.width / 2);

                        var arrowblock = blocks.filter(a => a.id == children.id)[0];
                        var arrowx = paddingx;
                        var arrowy = arrowblock.y - r_parent.y;
